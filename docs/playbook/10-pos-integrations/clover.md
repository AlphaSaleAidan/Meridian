...
**Partner program:** Meridian's Clover dev-dashboard app ("Meridian Ai Business Support", App ID `YK86AE2YAHSP2` — matches `CLOVER_APP_ID`) **requests write scopes as of 2026-07-20**: Orders, Customers, and Employees Read+Write; Inventory/Merchant/Payments Read. (Employees Write is unused by code — candidate to drop before App Market submission.) The app itself is still a **DRAFT** — no App Market release has ever been submitted (App Releases is empty, 0 merchant installs), so "pending approval" was inaccurate: nothing is in review yet. Production write for real merchants stays gated on submitting + passing App Market review. What DID change: any test/dev merchant that authorizes the draft app from now on grants the write scopes, which unblocks end-to-end testing of order injection (`clover_kitchen.py`). See [Clover — read-only today](clover-read-only.md) for the exact merchant-expectation language; never tell a merchant we're already approved.
