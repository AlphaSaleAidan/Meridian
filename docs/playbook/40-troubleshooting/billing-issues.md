...
| Quoted base product, billed with add-on charges | An add-on was triggered somewhere; check Settings → Subscription history |
...
## Symptom: "I want to remove an add-on / change my plan"

| Action | Effective when |
|--------|---------------|
| Remove an add-on (e.g. camera intelligence) | Next billing cycle (we don't pro-rate removals) |
| Add a new add-on | Immediate, pro-rated on next invoice |
| Pause | Not currently offered; cancel + restart later |

**Rep alert:** if a merchant wants to drop an add-on, **find out why first**. Often it's a feature they didn't realize they had (cameras, multi-location), or a single bad experience. Retention saves your commission.
...
## Symptom: "Charged for cameras but I didn't add that"

| Cause | Fix |
|-------|-----|
| Camera intelligence add-on was added inadvertently | Check subscription history; refund + remove add-on |
