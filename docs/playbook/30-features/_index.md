...
Meridian's AI agents grouped by category. Each agent is auto-generated from its source-of-truth class metadata in `src/ai/agents/`.

## POS Analytics agents (all included — transaction-only, no cameras needed)
...
| [location_analyst](./pos-analytics/location_analyst.md) | Multi-location comparison, best-practice transfer | 3 (multi-location add-on) |
...
## Vision (camera pipeline) — Camera add-on

Requires the **camera intelligence add-on** and at least one supported RTSP-capable camera. It's not packaged as discrete "agents" — it's a real-time pipeline running on each connected RTSP stream. See [vision/](./vision/) for the per-module breakdown.
...
## Cross-Reference (POS × Camera fusion) — Camera add-on

Requires the **camera intelligence add-on** + cameras connected + POS data. Nobody else does this POS × camera fusion at SMB scale — this is the add-on's key wedge.
...
## How features map to add-ons

See [_data-requirements-matrix.md](./_data-requirements-matrix.md) for which features need which data, from which POS systems.

| Add-on | POS analytics (all) | Vision pipeline | Cross-reference | Multi-location | API |
|--------|:-------------------:|:---------------:|:---------------:|:--------------:|:---:|
| Base product | Yes | — | — | — | — |
| + Camera intelligence | Yes | Yes | Yes | — | — |
| + Multi-location | Yes | — | — | Yes | — |
| + API + custom models | Yes | — | — | — | Yes |

The key upsell isn't more POS analytics — every merchant gets all 30. The add-on wedge is **camera intelligence + cross-reference fusion** (dwell-time-to-ticket correlation merchants can't get anywhere else), and **multi-location** for operators with multiple stores.
