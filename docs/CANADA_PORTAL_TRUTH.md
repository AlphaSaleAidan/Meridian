...
| Auto Dialer (2026-08-12, branch `feat/rep-autodialer`) | `/canada/portal/auto-dialer` — power dialer over the rep's OWN callable leads + due callbacks (single line, auto-advance, wrap-up countdown). Dial-time HARD gate server-side: internal `dialer_dnc` + CRTC calling windows (weekday 09:00–21:30, weekend 10:00–18:00, lead-local by area code); blocked attempts are logged, never dialed. Softphone = Telnyx WebRTC when `TELNYX_WEBRTC_CONNECTION_ID` is set, otherwise a clearly-badged SIM mode (no PSTN traffic). Data plane: `dialer_*` tables (`supabase/migrations/20260812_autodialer.sql`, hand-applied) or the in-memory dev store (`DIALER_DEV_STORE=1`, previews only). Spec: `docs/AUTODIALER_PLAN.md`. | Predictive/parallel dialing (CRTC abandonment regime); recording (no consent flow yet); dialing the unassigned pool; SIM mode presented as live calling |
| Phone-lead pool (2026-08-12, branch `feat/rep-autodialer`) | The Auto Dialer works a DEDICATED `canada_phone_leads` table (migration `20260812_canada_phone_leads.sql`, applied to live) — NOT `canada_leads`. Cold prospects with raw numbers never touch the pipeline. Reps load numbers via "Add leads" (one / bulk-paste); each call card shows enrichment (POS system, est $/mo, vertical, website). Recapture is automatic via `next_action_at` (no-answer +4h, callback → due time, dead/not-interested → out). "Meeting booked" → slot picker → books a `dialer_appointments` demo AND promotes the lead into `canada_leads` (stage `appointment_set`, `source: auto-dialer:*`) in one click — the ONLY write into the pipeline. In-dialer Calendar (Dialer\|Calendar toggle) + admin Console "Calendar" tab. | Dialing `canada_leads` directly; cold prospects inserted into `canada_leads`; promote writing anything but a clean qualified row |
| Call Console (2026-08-12, same branch) | `/canada/portal/call-console` — first real item under the sidebar's Admin section, "admin or better" only (page re-checks `repTier`/`isCanadaAdmin`; backend enforces `require_org_admin` on all `/api/dialer/admin/*`). Tabs: Live board (5s poll + Supabase Realtime on `dialer_sessions`/`dialer_calls`), Calls (post-hoc re-disposition, audited via `disposition_by`), Callbacks, DNC manager, Analytics. | Exposing the console to managers/reps; widening rep-scoped RLS "to fix the board" |
| Leaderboard (2026-07-17) | **Peer-visible by design, aggregate-only by design.** Non-admin Leaderboard tab reads `GET /api/leaderboard` (service-role query): ALL active reps in the caller's portal with ONLY `id, name, role, deals_won, deals_open, total_mrr` — no emails, phones, lead rows, or commission details. This is deliberately WIDER than the scoped roster (#334 collapsed the board to one for leaf reps — that was the regression). Admin Team Management stays on the scoped `/api/canada/team`. | Board rendered from the scoped roster (board of one); leaderboard endpoint returning emails/phones/lead rows; roster endpoints widened "to fix the board" |
