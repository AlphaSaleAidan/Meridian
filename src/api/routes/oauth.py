...
def _sign_state(org_id: str, rep_id: str | None = None) -> str:
    ...
    Format (P2 extension):
        org_id:rep_id:nonce:expires:sig
    rep_id is empty when the OAuth was initiated by a self-serve
    merchant (no concierge rep involved). Legacy 4-part tokens (no
    rep_id slot) are accepted by `_verify_state` for compatibility
    with any callbacks still in flight when this code rolls out.
    """
    nonce = uuid4().hex[:16]
...
def _verify_state(state: str) -> tuple[str, str | None] | None:
    ...
    Returns (org_id, rep_id_or_None) on success. Accepts both the new
    5-part format (`org_id:rep_id:nonce:expires:sig`) and the legacy
    4-part format (`org_id:nonce:expires:sig`) for inflight-callback
    compatibility — legacy tokens always return rep_id=None.
    """
    parts = state.split(":")
    ...
    if len(parts) == 5:
        ...
        # Legacy: org_id:nonce:expires:sig
    ...
    # Recompute the signed payload using the same shape the token was
    # signed with so legacy tokens still verify.
    if len(parts) == 5:
...
async def authorize(
    ...
    Query params:
      ...
      rep_id — optional concierge rep ID (sales_reps.id). When the
               connect flow is initiated by a rep on behalf of a
               merchant, we round-trip this through the signed state
               so the callback can write pos_connections.
               connected_by_rep_id without trusting an unsigned
               channel.
    """
    if not org_id:
    ...
    logger.info(
        f"OAuth: redirecting org {org_id} to Square authorize "
...
async def callback(
    ...
    try:
        ...
        if _db_instance:
            ...
            # Upsert POS connection (P2: rep attribution when state
            # carried a rep_id — only written when present so a
            # re-connect via a different surface doesn't clobber
            # existing attribution).
            # P6: schema-aligned column names. Schema uses
            # `external_merchant_id`, `access_token_enc`,
            # `refresh_token_enc`. The old `merchant_id`,
            # `access_token_encrypted`, `refresh_token_encrypted`
            # values were silently dropped by PostgREST on insert
            # (and read back as None), meaning OAuth-saved tokens
            # were unrecoverable.
            # `location_ids` (array) was also dropped — schema has
            # singular `external_location_id` (text). Leave it empty
            # at OAuth time; the backfill discovers locations.
            connection_data = {
                "id": str(uuid4()),
                "org_id": org_id,
                "provider": "square",
