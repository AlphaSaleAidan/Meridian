"""Short-lived HMAC tokens for the camera-connect flow (stdlib only — ponytail: no JWT dep).

Ported from the meridian-camera connector project. Two token types:

- **pairing code** (`mint_pairing_code`): shown in the portal "Connect cameras" wizard for the
  LAN-connector FALLBACK path. The connector exchanges it at `POST /api/connector/pair` for a
  device token. Stateless HMAC, short-lived — no pairings table needed.
- **stream token** (`mint_stream_token`): scoped to ONE camera, for a short-lived live view.

Everything fails CLOSED if `GATEWAY_JWT_SECRET` is not set.
"""
...
def mint_pairing_code(org_id: str, site_id: str, *, ttl_seconds: int = 900) -> str:
    """Wizard-shown pairing code (stateless HMAC; default 15 min). The LAN connector
    exchanges it at /api/connector/pair for a device token. ponytail: no pairings table."""
    secret = _secret()
    if not secret:
