...
- **Kotlin-owned DDL lives here:** Any schema change this backend needs (new tables, columns, indexes, defaults) is authored as a SQL file in `backend/supabase/migrations/` (e.g. `20260811_audit_metadata.sql`). **Never** add backend-driven schema changes to the repo-root `supabase/migrations/` — that folder is the wider Meridian product's historical, hand-applied migration set; nothing executes it as an ordered chain and it has no applied-state ledger.
- **Keep the mirrors in sync:** Until a real migration runner (e.g. Flyway) owns the schema, every migration must be mirrored by hand in (a) `backend/scripts/sql/init-local-db.sql` (local dev) and (b) the inline DDL/`ALTER ... IF NOT EXISTS` blocks in the integration tests that touch the affected tables. Write all of it idempotently (`IF NOT EXISTS`).
- **Applying to live is a human decision:** Migrations here are NOT auto-applied anywhere. Do not run them against any live/staging Supabase without explicit instruction; record in the migration header when/where it has been applied.
