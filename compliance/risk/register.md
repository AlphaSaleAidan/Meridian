...
| R-03a | **Phone webhooks unauthenticated** — no `X-Twilio-Signature`/`RequestValidator` anywhere in `src/` (confirmed); `/twilio/*` voice+status webhooks accept any POST → spoofable call/order events | Integrity/Spoofing | 4 | 4 | **16** | Mitigate | `CC6.7`; **R4** add provider signature validation (Telnyx + Twilio) | 🔴 OPEN |
| R-03b | **Raw PAN/CVV in memory (PCI)** — `phone.py:1573/1593/1612` capture DTMF card digits; **gated** behind `CARD_PAYMENT_ENABLED` (default False) + `pay_now` mode → likely inactive (pay-at-pickup norm) | Payment/PCI | 2 | 5 | **10** | Avoid | **R4** confirm `CARD_PAYMENT_ENABLED` is off in prod; if card-pay needed, use provider `<Pay>` so PAN never enters app memory | 🟡 latent (verify env) |
