...
|---|---|---|---|---|---|---|---|---|
| R-01 | **Cross-tenant data exposure via RLS `USING(true)`** on `vision_*`, `phone_agent_config`, `phone_orders`, `phone_call_logs`, `sms_optout_tracking`, `schedule_*` — any authenticated JWT can read other tenants' rows via PostgREST | Tenant isolation | 4 | 5 | **20** | Mitigate | `CC6.1-RLS`; fix migration authored `/evidence/CC6.1-RLS/`; **R0 confirm live state, R1/R3 apply** | 🔴 OPEN |
...
| R-04 | **Undisclosed secondary use (data resale)** — `resale_tier:"premium"` on camera/journey/transaction data not disclosed in SLA (only "improve the Services") | Privacy/Legal | 3 | 5 | **15** | Mitigate (legal) | `P-PRIVACY`; **R8** legal review of purpose disclosure | 🔴 OPEN |
