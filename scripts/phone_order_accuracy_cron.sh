#!/usr/bin/env bash
# Nightly wrong-order sweep for the Meridian phone agent.
# Checks the last few days of captured orders against their call transcripts,
# writes findings to phone_order_accuracy, and emails the flagged ones.
# NEVER modifies an order or the live agent — detection + review only.
#
# Install alongside the existing phone job (see `crontab -l`):
#   20 4 * * *  /root/Meridian/scripts/phone_order_accuracy_cron.sh
# Env is extracted from /root/Meridian/.env key-by-key because that file has
# unquoted values that break `source`.
set -uo pipefail

ENV_FILE=/root/Meridian/.env
OUT=/tmp/phone-order-accuracy
LOG=/tmp/meridian-phone-order-accuracy.log
...
echo "=== $(date -u +%FT%TZ) phone order-accuracy sweep ===" >> "$LOG"

# --days 3 gives overlap if a night is missed; already-checked calls are skipped.
run_out="$(timeout 1200 python3 scripts/phone_order_accuracy.py \
  --days 3 --concurrency 3 --out "$OUT" 2>&1)"
echo "$run_out" >> "$LOG"

# No new orders, or none flagged — stay quiet.
if echo "$run_out" | grep -q "nothing to do"; then
  echo "no new orders — skipping email" >> "$LOG"
  exit 0
fi
if [ ! -s "$OUT/flagged.jsonl" ]; then
  echo "no flagged orders — skipping email" >> "$LOG"
  exit 0
fi
...
def send(frm):
    req = urllib.request.Request("https://api.resend.com/emails",
        data=json.dumps({"from": frm, "to": [os.environ["REPORT_TO"]],
            "subject": "Meridian phone agent — orders flagged for review", "html": html}).encode(),
