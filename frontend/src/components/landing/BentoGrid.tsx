    ...
    visual: (
      <div className="mt-4 rounded-lg bg-[#0A0A0B] border border-[#1F1F23] p-4 font-mono text-sm">
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="text-[#7C5CFF] font-semibold">$2,340</span>
        </div>
        <div className="space-y-2">
          {[
            { label: 'Bundle pricing', value: '$890', pct: 38 },
            { label: 'Peak hour staffing', value: '$720', pct: 31 },
    ...
    visual: (
      <div className="mt-4 flex items-end gap-1 h-16">
        {[35, 42, 28, 56, 48, 62, 55, 70, 65, 78, 72, 85].map((h, i) => (
          <div
            key={i}
            className="flex-1 rounded-sm bg-[#7C5CFF]"
  ...
  },
  {
    ...
    visual: (
      <div className="mt-4 font-mono text-xs">
        <div className="flex items-center gap-2 text-[#4FE3C1]">
          <span>↑ 12.4%</span>
          <span className="text-[#A1A1A8]">vs last week</span>
        </div>
        <div className="text-2xl font-semibold text-[#F5F5F7] mt-1">$14,280</div>
  ...
  },
  {
    ...
    visual: (
      <div className="mt-4 space-y-2">
        {['Increase latte price by $0.50', 'Add morning pastry bundle'].map(text => (
          <div key={text} className="flex items-center gap-2 text-xs bg-[#0A0A0B] rounded-md px-3 py-2 border border-[#1F1F23]">
            <div className="w-1.5 h-1.5 rounded-full bg-[#7C5CFF] flex-shrink-0" />
    ...
    visual: (
      <div className="mt-4 text-xs space-y-2">
        ...
        <div className="flex items-center gap-2 text-[#A1A1A8]">
          <div className="w-2 h-2 rounded-full bg-[#4FE3C1]" />
        ...
        {['Connect', 'Analyze', 'Profit'].map((step, i) => (
          <div key={step} className="flex items-center gap-2">
            <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-medium ${
              i === 2 ? 'bg-[#7C5CFF] text-white' : 'bg-[#1F1F23] text-[#A1A1A8]'
            }`}>
              {i + 1}
            </div>
          ...
          >
            <div className="flex items-center gap-3 mb-2">
              <div className="w-8 h-8 rounded-lg bg-[#7C5CFF]/10 flex items-center justify-center">
                <f.icon size={16} className="text-[#7C5CFF]" />
              </div>
