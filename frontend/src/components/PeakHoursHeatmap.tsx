...
function GradientLegend() {
  return (
    <div className="flex items-center justify-center gap-2 mt-4">
      <span className="text-xs font-medium text-[#A1A1A8]/70">Low</span>
      <div className="flex gap-px">
        {LEGEND.map((c, i) => <div key={i} className="w-7 h-3 rounded-sm" style={{ backgroundColor: c }} />)}
      </div>
      <span className="text-xs font-medium text-[#A1A1A8]/70">High</span>
    </div>
  )
}
  ...
  if (single) {
    ...
    return (
      <div className="card p-4 sm:p-5" data-walkthrough="peak-heatmap">
        ...
        {caption && <p className="text-sm text-[#A1A1A8]/70 mb-3">{caption}</p>}
        <div className="space-y-2">
          {hours.map(hour => {
            ...
            return (
              <div key={hour} className="flex items-center gap-3" title={tip}>
                <span className="w-10 flex-shrink-0 text-right text-sm font-semibold text-white/90 tabular-nums">{hourLabels[hour]}</span>
                {/* thicker bar, length scales with demand (not full-width) inside a faint track */}
                <div className="flex-1 h-3 rounded-full bg-[#1F1F23] overflow-hidden">
                  <div className="h-full rounded-full" style={{ width: `${Math.max(normalized * 100, normalized > 0 ? 16 : 0)}%`, backgroundColor: cellColor(normalized) }} />
                </div>
              </div>
            )
          })}
