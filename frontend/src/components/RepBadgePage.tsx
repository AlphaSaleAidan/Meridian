...
function generateBadgeNumber(repId: string, prefix: string): string {
  let hash = 0
...
export interface RepBadgePageProps {
  /** Badge number prefix, e.g. "MRD-CA-" for Canada, "MRD-" for US */
  badgeNumberPrefix: string
  /** Brand name shown on card and used in share/vCard, e.g. "Meridian Canada" or "Meridian" */
  brandName: string
  /** Default title field value */
  defaultTitle: string
  /** Suffix appended to the downloaded PNG filename, e.g. "-ca" or "" */
  downloadSuffix: string
  /** Label shown under the logo on the card front, e.g. "Canada Sales" or "Sales Team" */
...
export default function RepBadgePage({
  badgeNumberPrefix,
  brandName,
  defaultTitle,
  downloadSuffix,
  ...
  const [badgeNumber, setBadgeNumber] = useState(() =>
    generateBadgeNumber(rep?.rep_id || 'demo', badgeNumberPrefix)
  )
  ...
  async function handleDownload() {
    ...
    try {
      ...
      link.download = `${name.replace(/\s+/g, '-').toLowerCase()}-badge${downloadSuffix}.png`
  ...
  async function handleShare() {
    if (navigator.share) {
      try {
        await navigator.share({ title: `${name} — ${brandName}`, url: badgeUrl })
      } catch { /* cancelled */ }
    } else {
