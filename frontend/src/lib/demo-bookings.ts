/**
 * The demo's booking backend, in the browser.
 *
 * /demo and /canada/demo have no merchant and no database, but Bookings is the
 * pillar the service trades open on — a barbershop demo without a book is not
 * a demo of this product. So the book is generated here: real resources, real
 * services, a day that fills at the trade's own rhythm, and writes that stick
 * for the session, so a prospect can confirm a guest, mark a no-show and see
 * the waitlist take the freed slot.
 *
 * IT ANSWERS ONLY ON A DEMO PATH. bookings-api.ts calls in here before it
 * reaches the network, and only when the URL says /demo — a signed-in merchant
 * never touches this file, because invented guests rendered inside a real
 * portal is the worst outcome available.
 *
 * The names, addresses and numbers are invented and are meant to be. They
 * exist to show the product, never to stand in for traction.
 *
 * Written to the same wire shape the real API returns (snake_case, UTC
 * instants, server-rendered localLabel) so the client code under test in the
 * demo is the same client code that runs in production.
 */
 ...
 *
 * The preview harness asks about 'preview-bookings'; the public demos ask
 * about 'demo'. Matching an exact id meant the demo's very first call —
 * /resources/demo — fell through to "no fixture" and the page rendered an
 * error. There is only ever one book in the browser, so the id is a label
 * rather than a key.
 */
const MERCHANT = 'preview-bookings'

/**
 * The demo merchant is placed in WHOEVER IS LOOKING's timezone.
 *
 ...
 * instead of letting the browser format the time.
 *
 * DO NOT PIN THIS TO A MARKET ZONE. It was tried: America/Vancouver for the
 * Canadian demo. The page derives the day it asks for from the BROWSER, while
 * the book seeds its day in TZ, so the moment those two clocks sit on
 * different calendar dates — any evening in Europe, which is where this was
 * caught — the page asks for a day the book never seeded and every trade
 * comes back empty. Aligning them is what makes the demo work anywhere.
 */
...
function seedDay(dayKey: string) {
  ...
  // Closed days come from THE SHOP'S OWN HOURS, which is the same row the
  // availability endpoint and the calendar read. Consulting the trade
  // template here instead left the two disagreeing: the shop advertised hours
  // today and the book seeded nothing into them, so a med spa opened on a
  // Sunday with an empty day and no explanation for it.
  const hours = HOURS.find((h) => h.weekday === weekday)
  if (!hours) return
  ...
  const [closeH] = hours.closes_at.split(':').map(Number)
...
export function configureForTrade(pack: any) {
  ...
  for (const svc of pack.services) {
    SERVICES.push({
      ...
      // Same figure the rest of the demo shows: /canada/demo is priced in
      // Canadian dollars, so a service costs what it would cost there rather
      // than a US price with a CA$ symbol in front of it.
      price_cents: svc.price == null ? null : Math.round(svc.price * getCurrencyMultiplier()), min_party: svc.min, max_party: svc.max, active: true,
    })
  }

  const open = (weekday: number) => HOURS.push({
    weekday,
  ...
  for (const weekday of pack.days) open(weekday)

  // THE DEMO SHOP TRADES TODAY, whatever day a prospect happens to arrive on.
  //
  // pack.days is the template's suggested week — Tuesday to Saturday for a
  // barbershop — and it is correct as a default for a real merchant setting
  // up. But a prospect who opens the demo on a Sunday would land on an empty
  // book and read it as broken rather than as closed. Plenty of real shops in
  // every one of these trades open seven days, so this is the demo merchant's
  // own hours rather than a claim about the trade.
  const todayWeekday = atLocal(todayKey, 12, 0).getUTCDay()
  if (!HOURS.some((h) => h.weekday === todayWeekday)) open(todayWeekday)
...
function dashboardRoute(url: URL): Response | null {
  ...
  return json({ detail: `demo dashboard: no route for ${path}` }, 404)
}
...
async function route(url: URL, init: RequestInit): Promise<Response> {
  ...
  // — resources —
  if (seg[0] === 'resources' && seg.length === 2 && method === 'GET') return json({ resources: RESOURCES })
  ...
  // — services —
  if (seg[0] === 'services' && seg.length === 2 && method === 'GET') return json({ services: SERVICES })
  ...
  // — hours —
  if (seg[0] === 'hours' && seg.length === 2 && method === 'GET') return json({ hours: HOURS })
...
/**
 * Answer a bookings request from the demo book.
 *
 * Returns a Response exactly as the network would, or null when the path is
 * not one this backend serves — the caller then falls through to the real
 * fetch rather than inventing a 404.
 */
/** Which trade the book currently holds, so it is rebuilt on a change and
 *  only on a change — reconfiguring on every request would wipe the writes a
 *  prospect just made. */
let configuredFor: string | null = null

function ensureConfigured() {
  const pack = packFor(getActiveBusinessType())
  if (configuredFor === pack.key) return
  configureForTrade(pack)
}

export async function demoBookingsRoute(
  url: URL,
  ...
  // a demo that renders instantly hides the states a real merchant will see.
  await new Promise((r) => setTimeout(r, 90 + Math.random() * 120))
  try {
    return await route(url, init as any)
  } catch (e) {
    return json({ detail: `demo bookings error: ${String(e)}` }, 500)
  }
}

/** The dashboard fixtures, for the preview harness only. */
export async function demoDashboardRoute(url: URL): Promise<Response | null> {
