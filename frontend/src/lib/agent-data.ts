...
export function generateAgents(): AgentInfo[] {
  return [
    ...
    { id: 'basket-analysis', name: 'Basket Analysis', status: 'active', lastRun: hoursAgo(1), nextRun: hoursFromNow(4), findings: 5, confidence: 83, category: 'analysis', description: 'Identifies co-purchase pairs and bundle recommendations from transaction line items', latestFinding: 'Espresso + Blueberry Muffin co-purchase rate 28% — potential $6.50 combo bundle' },
    { id: 'cash-flow', name: 'Cash Flow', status: 'active', lastRun: minutesAgo(30), nextRun: hoursFromNow(6), findings: 3, confidence: 80, category: 'forecasting', description: '14-day cash position forecast with processing fee analysis', latestFinding: 'Processing fees averaging 2.8% — switching to flat-rate plan could save $220/mo' },
    { id: 'cashflow-forecast', name: 'Cashflow Forecast', status: 'active', lastRun: hoursAgo(2), nextRun: hoursFromNow(6), findings: 2, confidence: 76, category: 'forecasting', description: '30-day cash flow projection with danger zone detection for low-balance periods', latestFinding: 'Cash position dips to danger zone around the 18th — schedule vendor payments after the 20th' },
    { id: 'category-mix', name: 'Category Mix', status: 'active', lastRun: hoursAgo(1), nextRun: hoursFromNow(5), findings: 4, confidence: 81, category: 'analysis', description: 'Analyzes category revenue share and identifies mix optimization opportunities', latestFinding: 'Drinks dominate at 68% revenue share — food category underperforming at 32% vs 40% benchmark' },
    { id: 'customer-ltv', name: 'Customer LTV', status: 'active', lastRun: hoursAgo(3), nextRun: hoursFromNow(6), findings: 4, confidence: 78, category: 'analysis', description: 'Predicts customer lifetime value and identifies churn risk by segment', latestFinding: 'Average LTV $1,840 — top decile customers worth $4,200+ over 12 months' },
    { id: 'customer-recognizer', name: 'Customer Recognizer', status: 'idle', lastRun: hoursAgo(24), nextRun: hoursFromNow(24), findings: 2, confidence: 70, category: 'analysis', description: 'Identifies repeat visitors via anonymized embeddings, tracks new vs returning ratio', latestFinding: '62% repeat visitor rate — loyalty-without-a-card metric trending up 5% MoM' },
    { id: 'day-of-week', name: 'Day of Week', status: 'active', lastRun: hoursAgo(2), nextRun: hoursFromNow(6), findings: 3, confidence: 87, category: 'analysis', description: 'Identifies best and worst performing days with day-specific revenue patterns', latestFinding: 'Saturday revenue 27% above average — Sunday underperforms by 21%, consider shorter hours' },
    { id: 'demographic-profiler', name: 'Demographic Profiler', status: 'idle', lastRun: hoursAgo(24), nextRun: hoursFromNow(24), findings: 2, confidence: 65, category: 'analysis', description: 'Detects age and gender distribution from camera feeds to inform marketing decisions', latestFinding: '25-34 age group represents 42% of traffic — tailor social media campaigns accordingly' },
    { id: 'discount-analyzer', name: 'Discount Analyzer', status: 'active', lastRun: hoursAgo(1), nextRun: hoursFromNow(4), findings: 4, confidence: 84, category: 'optimization', description: 'Evaluates discount ROI and detects cannibalization from over-discounting', latestFinding: 'Tuesday 20%-off promo cannibalized $180 in full-price sales — net ROI negative' },
    { id: 'dwell-time', name: 'Dwell Time', status: 'idle', lastRun: hoursAgo(24), nextRun: hoursFromNow(24), findings: 2, confidence: 69, category: 'analysis', description: 'Measures time visitors spend in each zone and builds browse-to-buy funnel metrics', latestFinding: 'Avg dwell time 4.2 min in browse zone — visitors spending 6+ min convert at 2.1x rate' },
    { id: 'growth-score', name: 'Growth Score', status: 'active', lastRun: hoursAgo(4), nextRun: hoursFromNow(8), findings: 1, confidence: 77, category: 'strategy', description: 'Calculates executive growth opportunity score 0-100 with monthly action plan', latestFinding: 'Growth score: 72/100 — pricing optimization and staffing adjustments are top levers' },
    { id: 'money-left', name: 'Money Left on Table', status: 'active', lastRun: hoursAgo(3), nextRun: hoursFromNow(6), findings: 3, confidence: 80, category: 'strategy', description: 'Aggregates all agent outputs into a headline dollar opportunity figure', latestFinding: '$3,840/mo total opportunity identified across pricing, staffing, and retention plays' },
    { id: 'payment-optimizer', name: 'Payment Optimizer', status: 'active', lastRun: hoursAgo(2), nextRun: hoursFromNow(6), findings: 3, confidence: 79, category: 'optimization', description: 'Analyzes payment method mix and identifies fee optimization opportunities', latestFinding: 'Cash transactions at 12% — encouraging cash for orders under $10 could save $160/mo in fees' },
    { id: 'pricing-power', name: 'Pricing Power', status: 'active', lastRun: hoursAgo(1), nextRun: hoursFromNow(5), findings: 4, confidence: 82, category: 'optimization', description: 'Measures price elasticity across SKUs and identifies safe price increase candidates', latestFinding: '4 items have inelastic demand (elasticity < -0.5) — combined price increase potential $1,200/mo' },
    { id: 'product-velocity', name: 'Product Velocity', status: 'active', lastRun: hoursAgo(1), nextRun: hoursFromNow(5), findings: 5, confidence: 86, category: 'analysis', description: 'Ranks products by sales velocity, identifies rising stars and dead stock', latestFinding: 'Cold Brew velocity up 18% MoM — rising star. Chai Latte declining 3 consecutive weeks' },
    { id: 'promo-roi', name: 'Promo ROI', status: 'active', lastRun: hoursAgo(4), nextRun: hoursFromNow(8), findings: 2, confidence: 75, category: 'optimization', description: 'Calculates promotion return on investment with diminishing returns detection', latestFinding: 'BOGO promo ROI at 1.4x — but repeat frequency shows diminishing returns after 3rd run' },
    { id: 'revenue-trend', name: 'Revenue Trend', status: 'active', lastRun: minutesAgo(20), nextRun: hoursFromNow(2), findings: 3, confidence: 88, category: 'analysis', description: 'Tracks daily, weekly, and monthly growth rates with trend signals', latestFinding: 'WoW growth +4.2%, MoM +8.3% — consistent uptrend, strongest in morning daypart' },
    { id: 'seasonality', name: 'Seasonality', status: 'active', lastRun: hoursAgo(6), nextRun: hoursFromNow(12), findings: 2, confidence: 73, category: 'forecasting', description: 'Decomposes seasonal patterns and calendar effects on revenue', latestFinding: 'Spring uptick begins mid-March historically — plan inventory ramp-up 2 weeks early' },
    { id: 'staffing', name: 'Staffing', status: 'active', lastRun: hoursAgo(2), nextRun: hoursFromNow(4), findings: 3, confidence: 81, category: 'optimization', description: 'Calculates optimal headcount per hour based on transaction volume and labor efficiency', latestFinding: 'Overstaffed by 1 during 2-4PM lull — reassigning to 7-9AM peak saves $18/day' },
    { id: 'waste-shrinkage', name: 'Waste & Shrinkage', status: 'active', lastRun: hoursAgo(3), nextRun: hoursFromNow(6), findings: 4, confidence: 80, category: 'optimization', description: 'Tracks shrinkage rate, void patterns, and projects annual loss from waste', latestFinding: 'Shrinkage rate 3.1% vs 2% target — void pattern on Register 2 needs investigation' },
    { id: 'foot-traffic', name: 'Foot Traffic', status: 'idle', lastRun: hoursAgo(24), nextRun: hoursFromNow(24), findings: 3, confidence: 72, category: 'analysis', description: 'Counts entries and exits from camera feeds, calculates visitor-to-transaction conversion', latestFinding: 'Conversion rate 34% — 66% of foot traffic leaves without purchasing, signage test recommended' },
    { id: 'queue-monitor', name: 'Queue Monitor', status: 'idle', lastRun: hoursAgo(24), nextRun: hoursFromNow(24), findings: 2, confidence: 71, category: 'optimization', description: 'Tracks real-time queue length and wait times, estimates walkaway revenue loss', latestFinding: 'Avg wait exceeds 3 min during 8-9AM — estimated $320/mo in walkaway losses' },
  ]
}
