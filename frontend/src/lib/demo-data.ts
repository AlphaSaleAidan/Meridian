...
function generateInsights(): { insights: Insight[]; total: number } {
  ...
  const insights: Insight[] = [
    ...
    },
    {
      id: uuid(),
      type: 'pricing',
      title: bizSub('📊 Gross Margin Compression: 58% → 54% Over 8 Weeks'),
      summary: bizSub('Your gross margin has eroded 4 points over the past 8 weeks — from 58.2% to 54.1%. At current revenue levels, that 4-point compression represents $1,120/month in lost profit.\n\n*Root cause analysis (Meridian AI):*\nCOGS increased 6.3% while menu prices remained flat. Investopedia\'s gross margin research notes that food-service businesses operating below 55% gross margin are in the "danger zone" where one bad month eliminates quarterly profit [Investopedia, 2026]. CFI analysis of 2,400 SMBs found that businesses monitoring margins weekly recover from compression 2.3x faster than those reviewing monthly [Corporate Finance Institute, 2026].\n\n*Recovery plan:*\n1. Audit your top 5 COGS items — identify which suppliers raised prices\n2. Renegotiate or find alternative suppliers for the 2 highest-impact ingredients\n3. Implement a 3-5% selective price increase on items with inelastic demand\n4. Target: Recover to 57%+ within 4 weeks'),
      details: {
    ...
    },
    {
      id: uuid(),
      type: 'product_recommendation',
      title: bizSub('🧪 Menu Engineering Matrix: 3 "Puzzles" Identified'),
      summary: bizSub('Meridian\'s menu engineering analysis classified your products into the Boston Consulting Group matrix. Three items are "Puzzles" — high profitability but low popularity: Matcha Latte ($2.80 margin, 3.1% of orders), Açaí Bowl ($3.40 margin, 2.8% of orders), and Turmeric Latte ($2.10 margin, 1.9% of orders).\n\n*Research-backed strategy:*\nCornell\'s menu engineering framework shows that repositioning Puzzles through strategic placement, staff recommendations, and bundling converts 30-50% into Stars within 4-6 weeks [Cornell Hospitality Quarterly, 2024]. McKinsey\'s consumer research confirms that menu item visibility drives 22% of customer selection — meaning placement changes alone can shift order patterns [McKinsey Consumer, 2025].\n\n*Conversion playbook:*\n1. Move all 3 Puzzles to the "golden triangle" on your menu board (top-right quadrant)\n2. Train staff to recommend one Puzzle per customer interaction\n3. Create a combo: Puzzle item + Star item at 10% bundle discount\n4. *Expected lift:* Convert 1-2 Puzzles to Stars = $340-$680/mo additional margin'),
      details: {
        puzzles: [
          { name: p[4]?.name || 'Item E', margin_cents: 280, order_pct: 3.1 },
          { name: p[7]?.name || 'Item H', margin_cents: 340, order_pct: 2.8 },
          { name: p[11]?.name || 'Item L', margin_cents: 210, order_pct: 1.9 },
        ],
        citations: ['cornell_menu_engineering', 'mckinsey_consumer_choice', 'nra_menu_trends'],
      },
      impact_cents: cx(51000),
      confidence: 0.71,
      action_status: 'pending',
      valid_until: daysFromNow(21),
    ...
    },
    {
      id: uuid(),
      type: 'anomaly',
      title: bizSub('🌧️ Weather Correlation Detected: Rain Days = -23% Revenue'),
      summary: bizSub('Meridian\'s pattern analysis detected a strong negative correlation between precipitation and your daily revenue. On days with >0.1" rainfall, revenue drops an average of 23% ($387 below daily mean).\n\n*Intelligence synthesis:*\nNation\'s Restaurant News reports that weather accounts for $22B in annual U.S. restaurant revenue variability [NRN, 2026]. Deloitte\'s retail analytics practice found that businesses with weather-responsive operations (adjusting staffing, promotions, and inventory based on forecasts) recover 40-60% of weather-related revenue losses [Deloitte Insights, 2026].\n\n*Weather playbook:*\n1. Monitor 3-day forecasts — reduce perishable prep by 20% on predicted rain days\n2. Launch "rainy day specials" via push notification or social media (warm drinks, comfort food)\n3. Cut one staff member on rain days to protect labor ratio\n4. Promote delivery/pickup options more aggressively when rain is forecast\n5. *Estimated recovery:* 40% of weather losses = $155/rain day'),
      details: {
        rain_impact_pct: -23,
        avg_loss_cents: cx(38700),
        rain_days_per_month: 6,
        recovery_potential_cents: cx(93000),
        citations: ['nrn_weather_impact', 'deloitte_retail_analytics', 'bls_seasonal_adjustment'],
      },
      impact_cents: cx(93000),
      confidence: 0.64,
      action_status: 'pending',
      valid_until: daysFromNow(14),
