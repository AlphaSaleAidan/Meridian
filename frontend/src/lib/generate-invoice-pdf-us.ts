// US invoice generator — a US-only fork of generate-invoice-pdf.ts.
// Differences from the Canada original: USD formatting, no Canadian HST/GST
...
export function generateInvoiceNumber(): string {
  ...
  return `MUS-${yy}${mm}-${uid8()}`
}

export function generateInvoiceUrl(invoiceNumber: string): string {
  return `${window.location.origin}/us/invoice/${encodeURIComponent(invoiceNumber)}`
}
...
export async function generateInvoicePdf(input: InvoiceInput): Promise<Blob> {
  ...
  doc.text('United States', margin, 25)
  ...
  doc.text(formatUSD(monthlyPriceCents), margin + contentW - 30, y, { align: 'right' })

  if (input.firstMonthFree) {
    doc.setTextColor(0, 212, 170)
    doc.text('$0.00 (FREE)', margin + contentW - 3, y, { align: 'right' })
  } else {
    doc.setTextColor(245, 245, 247)
    doc.text(formatUSD(monthlyPriceCents), margin + contentW - 3, y, { align: 'right' })
  }
  ...
  if (setupFeeCents > 0) {
    ...
    doc.setTextColor(170, 180, 175)
    doc.text('1', margin + contentW - 55, y, { align: 'right' })
    doc.text(formatUSD(setupFeeCents), margin + contentW - 30, y, { align: 'right' })
    doc.setTextColor(245, 245, 247)
    doc.text(formatUSD(setupFeeCents), margin + contentW - 3, y, { align: 'right' })
  ...
  // Totals — no Canadian HST/GST on US invoices; sales tax (where applicable)
  // is handled at checkout per state, not fabricated on this document.
  ...
  doc.setTextColor(245, 245, 247)
  doc.text(formatUSD(subtotal), margin + contentW - 3, y, { align: 'right' })
  ...
  doc.text(formatUSD(total), margin + contentW - 3, y + 2, { align: 'right' })
  ...
  if (input.recurring) {
    ...
    doc.text(
      ...
      `Next charge: ${formatUSD(monthlyPriceCents)} on ${nextMonth(input.billingDate)}.`,
      margin + 5, y + 10
    )
    y += 20
  }
  ...
  doc.setFontSize(7)
  doc.setTextColor(74, 85, 80)
  doc.text('Meridian Intelligence Inc. — United States', margin, footerY)
  doc.text(`Questions? Contact ${input.repName} at ${input.repEmail}`, margin, footerY + 4)
  doc.text('All prices in US Dollars (USD)', W - margin, footerY, { align: 'right' })
