...
function getAvatarColor(name: string): string {
  const colors = ['#17C5B0', '#f59e0b', '#7c3aed', '#ef4444', '#3b82f6', '#ec4899']
  let hash = 0
...
export default function USSalesLayout() {
  ...
  function handleLogout() {
    logout()
    navigate('/us/portal/login', { replace: true })
  }
          ...
          if (isHeading(entry)) {
            return (
              <p
                key={i}
                className={clsx(
                  'text-[10px] font-semibold text-[#A1A1A8] uppercase tracking-wider px-3 mb-1',
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                clsx(
                  ...
                  isActive
                    ? 'bg-[rgba(0,212,170,0.08)] text-[#17C5B0]'
                    : 'text-[#A1A1A8] hover:text-white hover:bg-[#1F1F23]/60'
                )
              }
            >
    ...
    <div className="flex h-screen overflow-hidden bg-[#0A0A0B]">
      ...
      <div className="flex-1 flex flex-col overflow-hidden bg-[#0A0A0B]">
        ...
        <header className="lg:hidden h-14 flex items-center gap-3 px-4 border-b border-[#1F1F23] bg-[#0A0A0B] flex-shrink-0">
          ...
          <button
            onClick={() => navigate('/us/portal/leads?new=true&t=' + Date.now())}
