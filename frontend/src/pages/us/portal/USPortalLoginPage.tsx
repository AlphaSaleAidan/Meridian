...
export default function USPortalLoginPage() {
  const navigate = useNavigate()
  const location = useLocation()
  const { authenticated, login, resetPassword } = useSalesAuth()

  const from = (location.state as { from?: string })?.from || '/us/portal/dashboard'
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="card p-6 sm:p-8 border border-[#1F1F23]">
          ...
          <p className="text-center text-[11px] text-[#A1A1A8] mt-5">
            New sales rep?{' '}
            <Link to="/us/portal/signup" className="text-[#17C5B0] hover:text-[#17C5B0]/80 transition-colors font-medium">
              Create your account
            </Link>
          </p>
        </div>
