...
const STATUS_STYLES: Record<string, { bg: string; text: string; label: string }> = {
  paid: { bg: 'bg-[#17C5B0]/10 border-[#17C5B0]/20', text: 'text-[#17C5B0]', label: 'Paid' },
...
export default function USPortalCommissionsPage() {
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-[#111113] border border-[#1F1F23] rounded-xl p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[10px] uppercase tracking-wider text-[#A1A1A8]">Total Earned</p>
        ...
        <div className="bg-[#111113] border border-[#1F1F23] rounded-xl p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[10px] uppercase tracking-wider text-[#A1A1A8]">Paid Out</p>
              <p className="text-lg font-bold text-[#17C5B0]">{formatCurrency(totalPaid)}</p>
            </div>
          </div>
        </div>
        <div className="bg-[#111113] border border-[#1F1F23] rounded-xl p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[10px] uppercase tracking-wider text-[#A1A1A8]">Pending Payout</p>
        ...
        <div className="bg-[#111113] border border-[#1F1F23] rounded-xl p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[10px] uppercase tracking-wider text-[#A1A1A8]">Your Rate</p>
        ...
        <div className="bg-[#111113] border border-[#1F1F23] rounded-xl p-4">
          ...
          <div className="space-y-2">
            <div className="flex justify-between items-center">
              <span className="text-[11px] text-[#A1A1A8]">This month</span>
              <span className="text-sm font-bold text-[#17C5B0]">{formatCurrency(Math.round(totalEarned * 1.15))}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-[11px] text-[#A1A1A8]">Next month (est.)</span>
              <span className="text-sm font-bold text-white">{formatCurrency(Math.round(totalEarned * 1.25))}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-[11px] text-[#A1A1A8]">Annual pace</span>
          ...
          {(() => {
            ...
            return (
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  <span className="text-[11px] text-[#A1A1A8]">Target: {formatCurrency(goal)}/mo</span>
                  <span className={`text-sm font-bold ${progress >= 100 ? 'text-[#17C5B0]' : 'text-[#7C5CFF]'}`}>{progress}%</span>
                </div>
                <div className="w-full h-2 bg-[#1F1F23] rounded-full overflow-hidden">
                  <div className={`h-full rounded-full transition-all ${progress >= 100 ? 'bg-[#17C5B0]' : 'bg-[#7C5CFF]'}`} style={{ width: `${progress}%` }} />
                </div>
                <p className="text-[10px] text-[#4a5550]">
                  {progress >= 100 ? 'Goal reached! Keep pushing for stretch targets.' : `~${dealsNeeded} more deal${dealsNeeded !== 1 ? 's' : ''} to hit $${goal.toLocaleString()}/mo`}
                </p>
              </div>
            )
          })()}
      ...
      <div className="flex flex-col sm:flex-row gap-3">
          ...
          {(['all', 'earned', 'pending', 'paid'] as const).map(f => (
            <button
              ...
              className={`px-3 py-1.5 text-xs font-medium rounded-lg transition-all capitalize ${
                filter === f ? 'bg-[#1F1F23] text-white' : 'text-[#A1A1A8] hover:text-white'
              }`}
            >
              {f === 'all' ? 'All' : f}
            </button>
        ...
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2 bg-[#111113] border border-[#1F1F23] rounded-xl text-sm text-white placeholder-[#4a5550] focus:outline-none focus:border-[#17C5B0]/50"
      ...
      <div className="space-y-2">
        {filtered.map(comm => {
          ...
          return (
            <div key={comm.id} className="bg-[#111113] border border-[#1F1F23] rounded-xl px-5 py-4">
              <div className="flex items-center gap-4">
                {/* Direction icon */}
                <div className={`w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 ${isIncome ? 'bg-[#17C5B0]/10' : 'bg-[#f0b429]/10'}`}>
                  {isIncome
                    ? <ArrowDownRight size={16} className="text-[#17C5B0]" />
                ...
                <div className="hidden sm:block text-right">
                  <p className="text-[11px] text-[#A1A1A8]">Gross: {formatCurrency(comm.gross_amount)}</p>
