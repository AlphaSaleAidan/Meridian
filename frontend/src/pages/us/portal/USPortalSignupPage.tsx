...
export default function USPortalSignupPage() {
  ...
  useEffect(() => {
    if (authenticated) navigate('/us/portal/dashboard', { replace: true })
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const err = await signup(name, email, password, phone || undefined, 'us')
    setLoading(false)
    if (err) {
      setError(err)
      return
    ...
    setSuccess(true)
    setTimeout(() => navigate('/us/portal/onboarding', { replace: true }), 1200)
  }
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="card p-6 sm:p-8 border border-[#1F1F23]">
          ...
          <p className="text-center text-[11px] text-[#A1A1A8] mt-5">
            ...
            <Link to="/us/portal/login" className="text-[#17C5B0] hover:text-[#17C5B0]/80 transition-colors font-medium">
              Sign in
            </Link>
          </p>
        </div>
