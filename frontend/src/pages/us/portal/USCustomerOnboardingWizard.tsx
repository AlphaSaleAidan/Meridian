...
const T = {
  pageBg:    'bg-[#0A0A0B]',
  cardBg:    'bg-[#111113]',
  cardBorder:'border-[#1F1F23]',
  inputBg:   'bg-[#0A0A0B]',
  inputBorder:'border-[#1F1F23]',
  accent:    '#17C5B0',
  accentBg:  'bg-[#17C5B0]',
  accentHover:'hover:bg-[#17C5B0]/90',
  accentTxt: 'text-[#17C5B0]',
  accentBorder:'border-[#17C5B0]',
  muted:     'text-[#A1A1A8]',
  text:      'text-[#F5F5F7]',
  focusBorder:'focus:border-[#17C5B0]/50',
} as const

const inputCls = `w-full px-3 py-2.5 text-[13px] rounded-lg ${T.inputBg} ${T.inputBorder} ${T.text} placeholder-[#A1A1A8]/40 ${T.focusBorder} focus:outline-none`
const btnPrimary = `flex items-center gap-2 px-6 py-2.5 text-[13px] font-medium ${T.accentBg} text-[#0A0A0B] rounded-lg ${T.accentHover} disabled:opacity-50 transition-colors`
...
export default function USCustomerOnboardingWizard() {
  ...
  const PROCESSING_KEY = 'meridian_us_processing_start'
  const PROCESSING_DONE_KEY = 'meridian_us_processing_done'
  ...
  async function handleAccountNext() {
    ...
    try {
      const meta: Record<string, string> = { country: 'US' }
  ...
  async function handleSlaSubmit() {
    ...
    try {
      ...
      const resp = await fetch(`${API_BASE}/api/us/sign-sla`, {
        method: 'POST',
        headers,
        body: JSON.stringify({
    ...
    <div className={`min-h-screen ${T.pageBg} flex flex-col items-center px-4 py-8`}>
      <div className="w-full max-w-xl">
        ...
        <div className="flex flex-col items-center gap-1 mb-6">
          ...
          <span className="text-[10px] font-semibold text-[#17C5B0] uppercase tracking-widest">US</span>
        </div>
            ...
            {STEPS.map((s, i) => {
              ...
              return (
                <div key={s.key} className="flex-1 flex flex-col gap-1.5">
                  <div className={`h-1 rounded-full transition-all duration-500 ${isDone ? 'bg-[#17C5B0]' : isActive ? 'bg-[#17C5B0]' : 'bg-[#1F1F23]'}`} />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input type="tel" value={account.phone} onChange={e => updateAccount('phone', e.target.value)}
                    placeholder="(555) 555-1234" className={inputCls} />
                </div>
              </div>
              <div>
                <label className={`block text-[11px] font-medium ${T.muted} mb-1.5`}>State</label>
          ...
          <div className="space-y-4">
            ...
            <div className={`${cardCls} max-h-96 overflow-y-auto space-y-3 text-[12.5px] ${T.text} leading-relaxed`}>
              ...
              <p className={T.muted}>Client data is collected, used, and disclosed in accordance with applicable United States
                federal and state privacy laws (including the California Consumer Privacy Act where applicable). Client retains
                ownership of all transaction data. Provider acts as a data processor only and will not sell or share Client data
                with third parties for advertising. Data is encrypted at rest and in transit (AES-256, TLS 1.3). Client may
                request access, correction, or deletion of personal information at any time by contacting <span className={T.text}>privacy@meridian.tips</span>.</p>

              <h3 className="text-[13px] font-semibold mt-3">4. Term &amp; Termination</h3>
              ...
              <h3 className="text-[13px] font-semibold mt-3">5. Limitation of Liability</h3>
              ...
              <h3 className="text-[13px] font-semibold mt-3">6. Governing Law</h3>
              <p className={T.muted}>This Agreement is governed by the laws of {province || 'the Client\'s state of residence'},
                United States. Disputes will be resolved in the courts of competent jurisdiction therein.</p>
          ...
          <div className="space-y-4">
            ...
            <div className={`${cardCls} space-y-4`}>
              <div className="flex items-center gap-3">
                <button onClick={() => fileInputRef.current?.click()}
                  className={`flex items-center gap-2 px-4 py-2.5 text-[12px] font-medium ${T.text} ${T.cardBg} rounded-lg hover:bg-[#1F1F23] ${T.cardBorder} transition-colors`}>
                ...
                {inventoryItems.map((item) => (
                  <div key={item.id} className="grid grid-cols-12 gap-2 items-center">
                    <input type="text" value={item.name} onChange={e => updateInventoryItem(item.id, 'name', e.target.value)}
                      placeholder="Item name" className={`col-span-3 px-2 py-2 text-[11px] rounded ${T.inputBg} ${T.inputBorder} ${T.text} placeholder-[#A1A1A8]/20 ${T.focusBorder} focus:outline-none`} />
                    <input type="text" value={item.category} onChange={e => updateInventoryItem(item.id, 'category', e.target.value)}
                      placeholder="Category" className={`col-span-2 px-2 py-2 text-[11px] rounded ${T.inputBg} ${T.inputBorder} ${T.text} placeholder-[#A1A1A8]/20 ${T.focusBorder} focus:outline-none`} />
                    <input type="number" value={item.costPerUnit} onChange={e => updateInventoryItem(item.id, 'costPerUnit', e.target.value)}
                      placeholder="$" className={`col-span-2 px-2 py-2 text-[11px] rounded ${T.inputBg} ${T.inputBorder} ${T.text} placeholder-[#A1A1A8]/20 ${T.focusBorder} focus:outline-none`} />
                    <input type="text" value={item.supplier} onChange={e => updateInventoryItem(item.id, 'supplier', e.target.value)}
                      placeholder="Supplier" className={`col-span-3 px-2 py-2 text-[11px] rounded ${T.inputBg} ${T.inputBorder} ${T.text} placeholder-[#A1A1A8]/20 ${T.focusBorder} focus:outline-none`} />
            ...
            <div className={`${cardCls} space-y-3`}>
              ...
              <div onClick={() => inventoryDocRef.current?.click()}
                className={`flex flex-col items-center justify-center py-8 cursor-pointer rounded-lg border-2 border-dashed ${T.cardBorder} hover:border-[#17C5B0]/30 transition-colors`}>
                <div className="w-12 h-12 rounded-full bg-[#17C5B0]/10 flex items-center justify-center mb-2">
                ...
                <div className="space-y-2">
                  {inventoryDocs.map((doc, idx) => (
                    <div key={idx} className={`flex items-center gap-3 px-3 py-2 rounded-lg ${T.cardBg} border ${T.cardBorder}`}>
                      ...
                      <button onClick={() => removeInventoryDoc(idx)} className="p-1 text-[#A1A1A8]/30 hover:text-red-400 transition-colors">
                        <X size={12} />
                      </button>
                    </div>
                  ))}
              ...
              {staffMembers.map((member) => (
                <div key={member.id} className="grid grid-cols-12 gap-2 items-center">
                  <input type="text" value={member.name} onChange={e => updateStaffMember(member.id, 'name', e.target.value)}
                    placeholder="Name" className={`col-span-4 px-3 py-2.5 text-[12px] rounded-lg ${T.inputBg} ${T.inputBorder} ${T.text} placeholder-[#A1A1A8]/20 ${T.focusBorder} focus:outline-none`} />
                  ...
                  <input type="number" value={member.hourlyRate} onChange={e => updateStaffMember(member.id, 'hourlyRate', e.target.value)}
                    placeholder="$/hr" className={`col-span-3 px-3 py-2.5 text-[12px] rounded-lg ${T.inputBg} ${T.inputBorder} ${T.text} placeholder-[#A1A1A8]/20 ${T.focusBorder} focus:outline-none`} />
            ...
            <div className={cardCls}>
              {schedulePreview ? (
                ...
                <div onClick={() => scheduleInputRef.current?.click()}
                  className={`flex flex-col items-center justify-center py-12 cursor-pointer rounded-lg border-2 border-dashed ${T.cardBorder} hover:border-[#17C5B0]/30 transition-colors`}>
                  <div className="w-14 h-14 rounded-full bg-[#17C5B0]/10 flex items-center justify-center mb-3">
              ...
              {paymentComplete && (
                <div className="w-full flex justify-center">
                  <button onClick={startProcessing}
                    className={`flex items-center gap-2 px-8 py-3 text-[14px] font-medium text-[#0A0A0B] ${T.accentBg} rounded-lg ${T.accentHover} transition-colors shadow-[0_0_30px_rgba(0,212,170,0.15)]`}>
