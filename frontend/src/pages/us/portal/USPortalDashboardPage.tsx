...
export default function USPortalDashboardPage() {
  ...
  useEffect(() => {
    usLeadsService.list(rep?.rep_id).then(d => {
      applyDeals(d)
    }).catch((err) => {
    ...
    const channel = usLeadsService.subscribe(rep?.rep_id, applyDeals)
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <QuickAction
          ...
          onClick={() => navigate('/us/portal/new-customer')}
        />
        <QuickAction
          icon={<GraduationCap size={16} />}
          label="View Training"
          onClick={() => navigate('/us/portal/training')}
        />
        <QuickAction
          icon={<MessageCircle size={16} />}
          label="Contact Manager"
          onClick={() => navigate('/us/portal/team')}
        />
      </div>
    </div>
  )
