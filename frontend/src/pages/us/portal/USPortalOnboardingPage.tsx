...
const REQUIRED_ITEMS = [
  { id: 'training', label: 'Complete all training modules', link: '/us/portal/training' },
  { id: 'product-overview', label: 'Read the Meridian product overview', link: '/us/portal/training' },
  { id: 'demo-practice', label: 'Practice the demo at least once', link: '/us/portal/leads?new=true' },
]
...
export default function USPortalOnboardingPage() {
  ...
  async function handleConfirmSend() {
    ...
    navigate('/us/portal/dashboard')
  }

  const inputClass = 'w-full px-3 py-2.5 bg-[#0A0A0B] border border-[#1F1F23] rounded-lg text-sm text-white placeholder-[#A1A1A8] focus:outline-none focus:border-[#17C5B0]/50 focus:ring-1 focus:ring-[#17C5B0]/20 transition-colors'
  const btnPrimary = 'flex items-center gap-1.5 px-5 py-2.5 bg-[#17C5B0] text-[#0A0A0B] text-sm font-semibold rounded-lg hover:bg-[#17C5B0]/90 transition-all'
  const btnSecondary = 'flex items-center gap-1.5 px-4 py-2.5 text-sm text-[#A1A1A8] hover:text-white transition-colors'
    ...
    <div className="min-h-screen bg-[#0A0A0B] flex flex-col items-center justify-center px-4 py-8">
      <div className="w-full max-w-lg">
        ...
        <div className="flex flex-col items-center gap-1 mb-6">
          ...
          <span className="text-[10px] font-semibold text-[#17C5B0] uppercase tracking-widest">
            US CRM
          </span>
        </div>
              ...
              <div key={s.id} className="flex items-center">
                <div
                  className={`w-8 h-8 rounded-full flex items-center justify-center transition-all ${
                    isActive ? 'bg-[#17C5B0] text-[#0A0A0B]' :
                    isDone ? 'bg-[#17C5B0]/20 text-[#17C5B0]' :
                ...
                {i < STEPS.length - 1 && (
                  <div className={`w-6 h-0.5 mx-0.5 ${i < stepIdx ? 'bg-[#17C5B0]/40' : 'bg-[#1F1F23]'}`} />
                )}
              </div>
            )
          })}
        ...
        {/* Card */}
        <div className="bg-[#111113] border border-[#1F1F23] rounded-xl p-6 sm:p-8">
          {/* Step 1: Password */}
          {step === 'password' && (
            <div className="space-y-5">
              <div className="text-center">
                <div className="w-12 h-12 rounded-full bg-[#17C5B0]/10 flex items-center justify-center mx-auto mb-3">
                  <Lock size={22} className="text-[#17C5B0]" />
                </div>
                <h2 className="text-lg font-bold text-white">Set Your Password</h2>
                <p className="text-sm text-[#A1A1A8] mt-1">Secure your account with a strong password.</p>
              </div>
              {passwordError && (
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  />
                  <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-[#A1A1A8]">
          ...
          {step === 'welcome' && (
            <div className="space-y-5">
              <div className="text-center">
                <div className="w-12 h-12 rounded-full bg-[#17C5B0]/10 flex items-center justify-center mx-auto mb-3">
                  <Sparkles size={22} className="text-[#17C5B0]" />
                </div>
                <h2 className="text-lg font-bold text-white">
                  Welcome to Meridian, {rep?.name?.split(' ')[0] || 'there'}
                </h2>
                <p className="text-sm text-[#A1A1A8] mt-1">
                  You're now part of the Meridian US sales team.
                </p>
              </div>
              ...
              <div className="grid grid-cols-3 gap-3">
                {[
                  ...
                  { label: 'Month 12', amount: '$30,000', sub: '60 closes' },
                ].map(card => (
                  <div key={card.label} className="bg-[#0A0A0B] border border-[#1F1F23] rounded-xl p-4 text-center">
                    <p className="text-[10px] font-medium text-[#A1A1A8] uppercase tracking-wide">{card.label}</p>
          ...
          {step === 'profile' && (
            <div className="space-y-5">
              <div className="text-center">
                <div className="w-12 h-12 rounded-full bg-[#17C5B0]/10 flex items-center justify-center mx-auto mb-3">
                  <User size={22} className="text-[#17C5B0]" />
                </div>
                <h2 className="text-lg font-bold text-white">Your Profile</h2>
                <p className="text-sm text-[#A1A1A8] mt-1">Fill in your details so your team and manager can find you.</p>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div className="sm:col-span-2">
                  ...
                  <select value={profile.state} onChange={e => setProfile(p => ({ ...p, state: e.target.value }))} className={inputClass}>
                    <option value="" disabled>Choose your state...</option>
                    {US_STATES.map(s => <option key={s} value={s}>{s}</option>)}
                  </select>
                </div>
                <div className="sm:col-span-2">
                  <label className="block text-xs font-medium text-[#A1A1A8] mb-1.5">Short Bio (optional)</label>
          ...
          {step === 'manager' && (
            <div className="space-y-5">
              <div className="text-center">
                <div className="w-12 h-12 rounded-full bg-[#17C5B0]/10 flex items-center justify-center mx-auto mb-3">
                  <Users size={22} className="text-[#17C5B0]" />
                </div>
                <h2 className="text-lg font-bold text-white">Meet Your Managers</h2>
                <p className="text-sm text-[#A1A1A8] mt-1">Here's who you'll be working with in US.</p>
              </div>
              <div className="space-y-3">
                <div className="bg-[#0A0A0B] border border-[#1F1F23] rounded-xl p-5">
                  <div className="flex items-center gap-4">
                    <div className="w-14 h-14 rounded-full bg-[#17C5B0]/20 flex items-center justify-center flex-shrink-0">
                      <span className="text-sm font-bold text-[#17C5B0]">EC</span>
                    </div>
                    <div>
                      <p className="text-sm font-semibold text-white">Enoch Cheung</p>
                      <p className="text-xs text-[#A1A1A8]">US Regional Director</p>
                      <p className="text-[10px] text-[#17C5B0] mt-0.5">Meridian AI Business Solutions — CRO</p>
                    </div>
                  </div>
                  <p className="text-xs text-[#A1A1A8] mt-4 leading-relaxed">
                    Enoch leads Meridian's US operations. He's your primary contact for pipeline strategy,
                ...
                <div className="bg-[#0A0A0B] border border-[#1F1F23] rounded-xl p-5">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-[#A1A1A8]">US Admin</p>
                      <p className="text-[10px] text-[#17C5B0] mt-0.5">Meridian</p>
                    </div>
                  </div>
                  <p className="text-xs text-[#A1A1A8] mt-4 leading-relaxed">
          ...
          {step === 'checklist' && (() => {
            ...
            return (
              <div className="space-y-5">
                <div className="text-center">
                  <div className="w-12 h-12 rounded-full bg-[#17C5B0]/10 flex items-center justify-center mx-auto mb-3">
                    <CheckSquare size={22} className="text-[#17C5B0]" />
                  </div>
                  <h2 className="text-lg font-bold text-white">Getting Started Checklist</h2>
                  <p className="text-sm text-[#A1A1A8] mt-1">Complete the required items to unlock your dashboard.</p>
                </div>

                {/* Progress bar */}
                <div>
                  <div className="flex items-center justify-between mb-1.5">
                    <span className="text-xs text-[#A1A1A8]">Required Progress</span>
                    <span className="text-xs font-semibold text-[#17C5B0]">{requiredDone}/{REQUIRED_ITEMS.length}</span>
                  </div>
                  <div className="h-2 bg-[#1F1F23] rounded-full overflow-hidden">
                    <div
                      className="h-full bg-[#17C5B0] rounded-full transition-all duration-500"
                ...
                {/* Required items */}
                <div>
                  <p className="text-[10px] font-semibold text-[#A1A1A8] uppercase tracking-wider mb-2">Required</p>
                    ...
                    {REQUIRED_ITEMS.map(item => {
                      ...
                      return (
                        <button
                          ...
                          className="w-full flex items-center gap-3 p-3 bg-[#0A0A0B] rounded-lg hover:bg-[#0A0A0B]/80 transition-colors text-left"
                        >
                          <div className={`w-5 h-5 rounded flex items-center justify-center flex-shrink-0 ${
                            checked ? 'bg-[#17C5B0] text-[#0A0A0B]' : 'border border-[#1F1F23]'
                          }`}>
                            {checked && <Check size={12} />}
                          </div>
                          <span className={`text-sm flex-1 ${checked ? 'text-[#A1A1A8] line-through' : 'text-white'}`}>
                            {item.label}
                          </span>
                        </button>
                      )
                ...
                {/* Recommended items */}
                <div>
                  <p className="text-[10px] font-semibold text-[#A1A1A8] uppercase tracking-wider mb-2">Recommended</p>
                  <div className="space-y-2">
                    {RECOMMENDED_ITEMS.map(item => {
                      ...
                      return (
                        <button
                          ...
                          className="w-full flex items-center gap-3 p-3 bg-[#0A0A0B]/50 rounded-lg hover:bg-[#0A0A0B]/80 transition-colors text-left"
                        >
                          <div className={`w-5 h-5 rounded flex items-center justify-center flex-shrink-0 ${
                            checked ? 'bg-[#17C5B0]/60 text-[#0A0A0B]' : 'border border-[#1F1F23]/60'
                          }`}>
                            {checked && <Check size={12} />}
                          </div>
                          <span className={`text-sm ${checked ? 'text-[#4a5550] line-through' : 'text-[#A1A1A8]'}`}>
                            {item.label}
                          </span>
                        </button>
                      )
                ...
                <div className="flex justify-between items-center">
                  ...
                  <div className="flex gap-2">
                    <Link to="/us/portal/training" className={btnSecondary + ' border border-[#1F1F23] rounded-lg px-4'}>
                      Go to Training
                    </Link>
                    <button
