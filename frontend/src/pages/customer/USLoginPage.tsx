...
export default function USLoginPage() {
  ...
  useEffect(() => {
    ...
    if (!org.onboarded) {
      navigate('/us/setup', { replace: true })
      return
    }
  ...
  async function handleForgot(e: React.FormEvent) {
    ...
    const { error: resetError } = await supabase.auth.resetPasswordForEmail(email, {
      redirectTo: window.location.origin + '/us/login',
    })
    setLoading(false)
    ...
    <div className="min-h-screen bg-[#0A0A0B] flex flex-col items-center justify-center px-4">
      <div className="w-full max-w-sm">
        ...
        <div className="card p-6 sm:p-8 border border-[#1F1F23]">
          ...
          {success && (
            <div className="mb-4 p-3 rounded-lg bg-[#1A8FD6]/10 border border-[#1A8FD6]/20 text-xs text-[#1A8FD6]">{success}</div>
          )}
          ...
          {mustReset ? (
            ...
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              <div className="flex items-center justify-between text-[11px]">
                ...
                <button type="button" onClick={() => navigate('/us/onboard')} className="text-[#17C5B0] hover:text-[#1A8FD6] transition-colors">
                  Create account
                </button>
              </div>
            </form>
