...
function generateBadgeNumber(repId: string): string {
  ...
  return `MRD-CA-${String(num).padStart(5, '0')}`
}
...
export default function CanadaPortalBadgePage() {
  const { rep } = useSalesAuth()
  const cardRef = useRef<HTMLDivElement>(null)

  const [name, setName] = useState(rep?.name || '')
  const [title, setTitle] = useState('Sales Representative — Canada')
  ...
  async function handleDownload() {
    ...
    try {
      ...
      link.download = `${name.replace(/\s+/g, '-').toLowerCase()}-badge-ca.png`
  ...
  async function handleShare() {
    if (navigator.share) {
      try {
        await navigator.share({ title: `${name} — Meridian Canada`, url: badgeUrl })
      } catch { /* cancelled */ }
    } else {
  ...
  function downloadVCard() {
    ...
    const vcf = [
      ...
      `TITLE:${title}`, `ORG:Meridian Canada`,
    ...
    <div className="space-y-8 pb-24 print:pb-0">
      ...
      <div>
        <h1 className="text-2xl font-bold text-white flex items-center gap-2">
          ...
          Digital Badge — Canada
        </h1>
        <p className="text-sm text-[#A1A1A8] mt-1">
          Create your professional Canadian business card with QR code
        </p>
      </div>
          ...
          <div className="bg-[#111114] border border-[#1F1F23] rounded-xl p-5 space-y-4">
            ...
            <div>
              ...
              <input
                ...
                className="w-full bg-[#0A0A0B] border border-[#2A2A2E] rounded-lg px-3 py-2.5 text-sm text-white focus:border-[#17C5B0] focus:ring-1 focus:ring-[#17C5B0]/30 outline-none transition-colors"
                placeholder="Sales Representative — Canada"
              />
            </div>
        ...
        <div className="flex flex-col items-center gap-4">
          ...
          >
            ...
            <div className="relative h-full flex flex-col items-center justify-center p-6 text-center">
              ...
              <div className="text-white text-xl font-bold">Meridian Canada</div>
