 ...
 * exceptions, and exercises critical add-lead / new-customer sequences.
 *
 * Run:
 *   E2E_APP_URL=http://209.126.80.45:3000 \
 ...
 *
 * Report written to: /tmp/ui-smoke-report.json
 */
...
// Selectors / text patterns that we must NOT click (destructive or navigates
// us out of the portal and breaks the crawl).
const SKIP_PATTERNS = [
  /sign\s*out/i,
  /log\s*out/i,
  /^delete/i,
...
async function attachListeners(page: Page, portal: 'us' | 'canada', getRoute: () => string) {
  ...
  page.on('response', (res) => {
    ...
    if (status >= 400 && status < 500) {
      // 401 on auth endpoints during login attempts is normal
...
async function login(page: Page, portal: 'us' | 'canada') {
  ...
  // Wait for the email input to appear; both portals use type=email
...
async function crawlRoute(page: Page, portal: 'us' | 'canada', route: string) {
  ...
  try {
    ...
    // Allow lazy chunks + initial data fetches to settle; cap short so a hung
    // long-poll/realtime socket doesn't burn the whole budget on one route.
  ...
  // Collect interactive elements: buttons and role=button anchors.
  // We do not click <a href> that would navigate away — we'll cover routes
  // explicitly via the loop above.
  ...
  for (const c of candidates) {
    ...
    // Re-resolve the element each iteration; the DOM may have changed after
    // the previous click (modal opened, list re-rendered, etc.).
    try {
      ...
      // Close modal via Escape if one opened (covers dialogs and menus)
      await page.keyboard.press('Escape').catch(() => {})
      await page.waitForTimeout(50)
      // If we accidentally navigated out of the portal, snap back
      if (!page.url().includes(`/${portal}/portal/`)) {
        await page.goto(url, { waitUntil: 'domcontentloaded' }).catch(() => {})
      }
    } catch (e: any) {
      const msg = e?.message ?? String(e)
      // Ignore "Element is not visible" and similar benign mid-flight failures
...
async function tryAddLeadSequence(page: Page, portal: 'us' | 'canada') {
  ...
  try {
    ...
    // The New Lead form lives inside a modal. Scope all interactions to the
    // modal so we don't accidentally hit the page-level search input. The
    // modal opens via ?new=true on first render, but if it didn't render we
    // click the "+ New Lead" button to force it open.
    ...
    // Fill text/email/phone inputs by placeholder match (scoped to modal)
    ...
    // Set every <select> inside the modal to its first non-empty option. This
    // covers required selects (POS system, vertical, source, state) without
    // hardcoding labels. selectOption('') fails, so we pick index 1+.
    ...
    if (resp && resp.status() >= 400) {
      ...
      // No POST after a real click means HTML5 validation still blocked
      // submit, or the network call is going somewhere else. Capture which
      // required field(s) are still :invalid so the user can see why.
...
test.describe.serial('UI smoke scrape', () => {
  ...
  for (const portal of ['us', 'canada'] as const) {
    test(`${portal} portal — login + crawl every route + add-lead sequence`, async ({ page }) => {
      ...
      // Test "passes" iff no sequence-fail / page-error / 5xx for this portal.
      // We assert non-empty report so CI surfaces issues count even on pass.
