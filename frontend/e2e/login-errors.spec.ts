 ...
 * customer), exercise three failure modes:
 ...
 *   npx playwright test e2e/login-errors.spec.ts --reporter=list
 */
...
function attach(page: Page, surface: string, getCase: () => string) {
  ...
  page.on('console', (m) => {
    if (m.type() === 'error') {
      const t = m.text()
      // Login pages legitimately surface "Invalid login credentials" 400 from
      // Supabase — that's the expected failure path. Filter it.
...
async function findErrorBanner(page: Page): Promise<string> {
  // Heuristic: look for any element with text-red/border-red class or aria-live
  const candidates = [
    '[role="alert"]',
    '[aria-live="polite"]',
    '[aria-live="assertive"]',
...
async function testEmptySubmit(page: Page, surface: { name: string; path: string }, caseName: string) {
  ...
  // Click submit without filling — HTML5 validation should block submission
  const submit = page.locator('button[type="submit"]').first()
  await submit.click().catch(() => {})
  await page.waitForTimeout(500)
  // Check email input :invalid pseudo-class
  const emailInput = page.locator('input[type="email"]').first()
  const isInvalid = await emailInput.evaluate((el: HTMLInputElement) => !el.checkValidity()).catch(() => false)
  if (!isInvalid) {
    // Either the form has no HTML5 validation (then expect an inline error),
    // or it actually submitted with empty creds (real bug).
...
async function testWrongPassword(page: Page, surface: { name: string; path: string; expectedAuthEndpoint: string }, caseName: string) {
  ...
  // Expect a visible error banner now
  await page.waitForTimeout(800)
  const banner = await findErrorBanner(page)
  if (!banner) {
