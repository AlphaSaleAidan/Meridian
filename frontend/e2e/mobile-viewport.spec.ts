/**
 * Mobile-viewport pass of the rep portal at iPhone SE dimensions (375x667).
 * Surfaces CSS bugs that only manifest below Tailwind's sm: breakpoint
 * (640px) — mobile nav, hamburger menu, dialog overlays, etc.
 *
 * Walks the same routes as ui-smoke but in a mobile context. Read-only:
 * skips add-lead and any mutation. Also asserts no horizontal overflow.
 */
...
// devices declaration is hoisted into the test block via test.use INSIDE the
// describe.serial so the iPhone SE viewport applies cleanly to both tests.
