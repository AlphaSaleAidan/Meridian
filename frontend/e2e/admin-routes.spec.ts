/**
 * Admin routes coverage: logs in as an admin email and crawls /customer/admin,
 * /admin/it-health, /admin/pos-coverage, /admin/email. Each is a protected
 * surface that's never exercised by the rep-portal smoke tests.
 *
 * For each route: assert it loads, click every non-destructive button, capture
 * console errors + network 4xx/5xx + page exceptions.
 *
 * Run:
 *   E2E_APP_URL=http://127.0.0.1:4173 \
 *   E2E_EMAIL=... E2E_PASSWORD=... \
 *   npx playwright test e2e/admin-routes.spec.ts --reporter=list
 */
...
const SKIP_PATTERNS = [
  ...
  /reject/i, /approve/i,             // admin actions on real users — destructive
  /disable/i, /deactivate/i,
  /reset.*password/i, /^email\b/i,    // would trigger real emails
]
...
async function loginAsAdmin(page: Page) {
  // Admin routes use the customer-side ProtectedRoute, which expects a logged-in
  // session from /customer/login. Sales reps don't have access. The admin
  // email also satisfies useAuth's admin gate for /customer/admin.
  await page.goto(`${APP_URL}/customer/login`, { waitUntil: 'domcontentloaded' })
  await page.waitForSelector('input[type="email"]', { timeout: 15_000 })
  await page.fill('input[type="email"]', EMAIL)
  await page.fill('input[type="password"]', PASSWORD)
  await page.click('button[type="submit"]')
  // Don't waitForURL — auth may redirect to / or /app/* depending on role.
...
async function crawl(page: Page, route: string) {
  ...
  // If we got bounced to a login page, that's a route-blocked, not a render bug.
