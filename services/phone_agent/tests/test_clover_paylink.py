...
def _native_pay_env(monkeypatch):
    """create_checkout's clover branch sits behind the global
    CLOVER_NATIVE_PAY_ENABLED kill-switch (second independent gate). ON here
    so the per-merchant toggle under test is what routes; the both-directions
    proof lives in test_env_kill_switch_blocks_clover_alone."""
...
@aio
async def test_env_kill_switch_blocks_clover_alone(monkeypatch, capture):
    """Independent gates, both directions: merchant toggle ON but env OFF →
    the clover branch must not run (default checkout path serves the order)."""
    ...
    async def fail_clover(*a, **k):
        raise AssertionError("clover branch must not run with the env gate off")
    monkeypatch.setattr(pl, "_merchant_has_clover", fail_clover)

    async def stripe(*a, **k):
        return {"url": "s", "method": "stripe"}
    monkeypatch.setattr(pl, "_stripe_checkout", stripe)

    out = await pl.create_checkout(ORDER, _cfg(), "ord_env")
